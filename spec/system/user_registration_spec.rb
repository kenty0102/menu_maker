...
RSpec.describe "ユーザー登録機能", type: :system do
  ...
  context "when 正しい情報が入力された場合" do
    it "登録後に自動でログインすること" do
      register_user("testuser", "rspec@example.com", "password", "password")
  ...
  context "when 不正な情報が入力された場合" do
    it "ユーザー名が無効な場合は登録できない" do
      expect do
        register_user("", "rspec@example.com", "password", "password")
    ...
    it "パスワードが無効な場合は登録できない" do
      expect do
        register_user("testuser", "rspec@example.com", "abc", "abc")
      end.not_to change(User, :count)
    end

    it "パスワードとパスワード確認が一致しない場合は登録できない" do
      expect do
        register_user("testuser", "rspec@example.com", "password", "12345678")
      end.not_to change(User, :count)
    end
